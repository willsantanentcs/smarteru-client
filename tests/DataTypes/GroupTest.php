...
class GroupTest extends TestCase {
    ...
    public function testExceptionIsThrownWhenTagNotInstanceOfTag() {
        ...
        self::expectExceptionMessage('Parameter to CBS\SmarterU\DataTypes\Group::setTags must be a list of Tag instances');
        $group = (new Group())
    ...
    public function testExceptionIsThrownWhenEmailAddressIsNotString() {
        self::expectException(InvalidArgumentException::class);
        self::expectExceptionMessage('Parameter to CBS\SmarterU\DataTypes\Group::setNotificationEmails must be a list of email addresses as strings');
        $group = (new Group())
            ->setNotificationEmails([1, 2, 3]);
    }

    /**
     * Tests that an exception is thrown if the group is translated to XML
     * without first setting the Account API Key.
     */
    public function testExceptionIsThrownWhenNoAccountAPIKey() {
        ...
        $this->group->toSimpleXML(null, null, 'method');
    }

    /**
     * Tests that an exception is thrown if the group is translated to XML
     * without first setting the User API Key.
     */
    public function testExceptionIsThrownWhenNoUserAPIKey() {
        self::expectException(MissingValueException::class);
        self::expectExceptionMessage('User API key must be set before creating a query.');
        $this->group->toSimpleXML('account', null, 'method');
    }
    ...
    public function testExceptionIsThrownWhenTagIsIncomplete() {
        ...
        $group->toSimpleXML('account', 'user', 'method');
    }
    ...
    public function testExceptionIsThrownWhenUserIdentifierNotSet() {
        ...
        self::expectException(MissingValueException::class);
        self::expectExceptionMessage('Each member of the group must be identified by either email or employee ID');
        $group->toSimpleXML('account', 'user', 'method');
    }
    ...
    public function testXMLGeneratedIsAsExpected() {
        ...
        $userApi = 'user';
        $method = 'method';
        $xml = $this->group->toSimpleXML($accountApi, $userApi, $method);
        ...
        self::assertEquals($this->group->getName(), $xml->Parameters->Group->Name);
        self::assertContains('GroupID', $groupTags);
        self::assertEquals($this->group->getGroupId(), $xml->Parameters->Group->GroupID);
        self::assertContains('Status', $groupTags);
        self::assertEquals($this->group->getStatus(), $xml->Parameters->Group->Status);
        self::assertContains('Description', $groupTags);
        self::assertEquals($this->group->getDescription(), $xml->Parameters->Group->Description);
        self::assertContains('HomeGroupMessage', $groupTags);
        self::assertEquals($this->group->getHomeGroupMessage(), $xml->Parameters->Group->HomeGroupMessage);
        ...
        self::assertEquals(count($emails), count($this->group->getNotificationEmails()));
        foreach ($emails as $email) {
            self::assertContains($email, $this->group->getNotificationEmails());
        }
        self::assertContains('UserHelpOverrideDefault', $groupTags);
        self::assertEquals($this->group->getUserHelpOverrideDefault() ? '1' : '0', (string) $xml->Parameters->Group->UserHelpOverrideDefault);
        self::assertContains('UserHelpEnabled', $groupTags);
        self::assertEquals($this->group->getUserHelpEnabled() ? '1' : '0', (string) $xml->Parameters->Group->UserHelpEnabled);
        self::assertContains('UserHelpEmail', $groupTags);
        self::assertEquals(implode(',', $this->group->getUserHelpEmail()), $xml->Parameters->Group->UserHelpEmail);
        self::assertContains('UserHelpText', $groupTags);
        self::assertEquals($this->group->getUserHelpText(), $xml->Parameters->Group->UserHelpText);
        ...
        self::assertEquals($tags[0]['TagID'], $this->group->getTags()[0]->getTagId());
        self::assertArrayHasKey('TagValues', $tags[0]);
        self::assertEquals($tags[0]['TagValues'], $this->group->getTags()[0]->getTagValues());
        self::assertIsArray($tags[1]);
        self::assertCount(2, $tags[1]);
        self::assertArrayHasKey('TagID', $tags[1]);
        self::assertEquals($tags[1]['TagID'], $this->group->getTags()[1]->getTagId());
        self::assertArrayHasKey('TagValues', $tags[1]);
        self::assertEquals($tags[1]['TagValues'], $this->group->getTags()[1]->getTagValues());
        ...
        self::assertEquals($this->group->getUserLimitEnabled() ? '1' : '0', (string) $xml->Parameters->Group->UserLimit->Enabled);
        self::assertContains('Amount', $limitTags);
        self::assertEquals($this->group->getUserLimitAmount(), (int) $xml->Parameters->Group->UserLimit->Amount);
        $users = [];
        ...
        self::assertEquals($users[0]['EmployeeID'], $this->group->getUsers()[0]->getEmployeeId());
        self::assertEquals($users[0]['HomeGroup'], $this->group->getUsers()[0]->getHomeGroup() ? '1' : '0');
        $codeSegment = (array) $users[0]['Permissions'];
        $codes = $codeSegment['Code'];
        self::assertEquals(count($codes), count($this->group->getUsers()[0]->getPermissions()));
        foreach ($this->group->getUsers()[0]->getPermissions() as $permission) {
            self::assertContains($permission->getCode(), $codes);
        }

        self::assertEquals($users[1]['EmployeeID'], $this->group->getUsers()[1]->getEmployeeId());
        self::assertEquals($users[1]['HomeGroup'], $this->group->getUsers()[1]->getHomeGroup() ? '1' : '0');
        $codeSegment = (array) $users[1]['Permissions'];
        self::assertEquals(count($codeSegment), count($this->group->getUsers()[1]->getPermissions()));
        ...
        self::assertEquals(count($modules), count($this->group->getLearningModules()));
        ...
        self::assertEquals($modules[0]['ID'], $this->group->getLearningModules()[0]->getId());
        self::assertEquals($modules[0]['AllowSelfEnroll'], $this->group->getLearningModules()[0]->getAllowSelfEnroll() ? '1' : '0');
        self::assertEquals($modules[0]['AutoEnroll'], $this->group->getLearningModules()[0]->getAutoEnroll() ? '1' : '0');
        self::assertEquals($modules[1]['ID'], $this->group->getLearningModules()[1]->getId());
        self::assertEquals($modules[1]['AllowSelfEnroll'], $this->group->getLearningModules()[1]->getAllowSelfEnroll() ? '1' : '0');
        self::assertEquals($modules[1]['AutoEnroll'], $this->group->getLearningModules()[1]->getAutoEnroll() ? '1' : '0');
        ...
        self::assertEquals($variants[0]['ID'], $this->group->getSubscriptionVariants()[0]->getId());
        self::assertEquals($variants[0]['RequiresCredits'], $this->group->getSubscriptionVariants()[0]->getRequiresCredits() ? '1' : '0');
        self::assertEquals($variants[1]['ID'], $this->group->getSubscriptionVariants()[1]->getId());
        self::assertEquals($variants[1]['RequiresCredits'], $this->group->getSubscriptionVariants()[1]->getRequiresCredits() ? '1' : '0');
        self::assertContains('DashboardSetID', $groupTags);
        self::assertEquals($xml->Parameters->Group->DashboardSetID, $this->group->getDashboardSetId());
    }

    /**
     * Test that XML generation produces the expected result when all required
     * values but no optional values are present.
     */
    public function testXMLOutputWithoutOptionalValues() {
        ...
        $xml = simplexml_load_string($group->toSimpleXML($accountApi, $userApi, $method));
        self::assertEquals('SmarterU', $xml->getName());
        $tags = [];
        foreach ($xml->children() as $tag) {
            $tags[] = $tag->getName();
        ...
        self::assertContains('Status', $groupTags);
        self::assertEquals($group->getStatus(), $xml->Parameters->Group->Status);
        self::assertContains('Description', $groupTags);
        self::assertEquals($group->getDescription(), $xml->Parameters->Group->Description);
        self::assertContains('HomeGroupMessage', $groupTags);
        self::assertEquals($group->getHomeGroupMessage(), $xml->Parameters->Group->HomeGroupMessage);
        self::assertContains('NotificationEmails', $groupTags);
        $emails = [];
        foreach ((array) $xml->Parameters->Group->NotificationEmails->NotificationEmail as $email) {
            $emails[] = $email;
        }
        self::assertEquals(count($emails), count($group->getNotificationEmails()));
        foreach ($emails as $email) {
            self::assertContains($email, $group->getNotificationEmails());
        }
        $users = [];
        ...
        self::assertEquals($users[0]['EmployeeID'], $group->getUsers()[0]->getEmployeeId());
        self::assertEquals($users[0]['HomeGroup'], $group->getUsers()[0]->getHomeGroup() ? '1' : '0');
        $codeSegment = (array) $users[0]['Permissions'];
        $codes = $codeSegment['Code'];
        self::assertEquals(count($codes), count($group->getUsers()[0]->getPermissions()));
        foreach ($group->getUsers()[0]->getPermissions() as $permission) {
            self::assertContains($permission->getCode(), $codes);
        }

        self::assertEquals($users[1]['EmployeeID'], $group->getUsers()[1]->getEmployeeId());
        self::assertEquals($users[1]['HomeGroup'], $group->getUsers()[1]->getHomeGroup() ? '1' : '0');
        $codeSegment = (array) $users[1]['Permissions'];
        self::assertEquals(count($codeSegment), count($group->getUsers()[1]->getPermissions()));
        ...
        self::assertEquals($modules[0]['ID'], $group->getLearningModules()[0]->getId());
        self::assertEquals($modules[0]['AllowSelfEnroll'], $group->getLearningModules()[0]->getAllowSelfEnroll() ? '1' : '0');
        self::assertEquals($modules[0]['AutoEnroll'], $group->getLearningModules()[0]->getAutoEnroll() ? '1' : '0');
        self::assertEquals($modules[1]['ID'], $group->getLearningModules()[1]->getId());
        self::assertEquals($modules[1]['AllowSelfEnroll'], $group->getLearningModules()[1]->getAllowSelfEnroll() ? '1' : '0');
        self::assertEquals($modules[1]['AutoEnroll'], $group->getLearningModules()[1]->getAutoEnroll() ? '1' : '0');
    }
}
